...
import com.darkprograms.speech.unit.Response;
...
public class Recognizer {
     ...
     */
    private static final String GOOGLE_RECOGNIZER_URL = "https://www.google.com/speech-api/v2/recognize?output=json&key=AIzaSyCJ0Dd_ZxaCtoU-LMGTy6IYsnavgQqaFIw";
    ...
    private void parseResponse(String rawResponse, GoogleResponse googleResponse) {
        Response response = new Gson().fromJson(rawResponse, Response.class);
        if (response != null && response.getResult() !=null)
        response.getResult().forEach(
                (in) -> in.getAlternative().
                        forEach((alternative) -> {
                            if (googleResponse.getResponse() == null){
                                googleResponse.setResponse(alternative.getTranscript());
                                if (alternative.getConfidence() != null) {
                                    googleResponse.setConfidence(alternative.getConfidence());
                                }
                            }else {
                                googleResponse.addPossibleResponse(alternative.getTranscript());
                            }
                        })
        );
    }
